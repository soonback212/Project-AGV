...
class IMUParserNode(Node):
    def __init__(self):
        ...
        self.ser = serial.Serial('/dev/ttyACM1', baudrate=115200, timeout=0.1)
    ...
    def read_serial_data(self):
        while self.ser.in_waiting >= 26:
            ...
            if sync[0] == 0xF5 and sync[1] == 0xF5:
                data = self.ser.read(24)
                if len(data) != 24:
                    self.get_logger().warn("데이터 길이 불완전")
                    continue
                ...
                imu_msg.orientation = Quaternion()  # 필요 시 계산
